...
1. `http.host eq "js.maytes.co"` and (`/v*.*.*/checkout-button.*` **or** (`/checkout-button.*.*` and not one of the 6 known unhashed bundle/map paths), all `wildcard`/`eq`) → `Cache-Control: public, max-age=31536000` (edge + browser TTL both one year)
...
**A real bug this design already had, caught by review before it shipped:** `/checkout-button.*.*` (the hash-pin pattern) requires two literal dots, which correctly excludes the bare unhashed `checkout-button.js`/`.mjs`/`.cjs` (one dot) — but their own unhashed source maps, `checkout-button.js.map` etc. (two dots: `.js` + `.map`), have the *same* dot count as a genuinely hashed file like `checkout-button.8c71493e.js`, so the wildcard alone can't tell them apart. Those source maps are exactly as mutable as the bare bundles they belong to (`scripts/hash-and-sri.mjs` copies both unhashed, unchanged, on every build) and must not get a year-long cache. Fixed by explicitly excluding the 6 known unhashed bundle/map paths (`UNHASHED_BUNDLE_PATHS` in `scripts/lib/cdn-cache-rules.mjs`) from the hash-pin match, via `eq`, rather than relying on the wildcard's dot-count alone.
