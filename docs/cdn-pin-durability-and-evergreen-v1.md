...
**Fix:** stop declaring Cache-Control in the generated `_headers` file for anything release-shaped. Instead, provision exactly **2 fixed Cloudflare Ruleset Engine Cache Rules**, matched against the URL shape rather than enumerated per release, via the Cloudflare API:

1. `http.host eq "js.maytes.co"` and (`/v*.*.*/checkout-button.*` **or** (`/checkout-button.*.*` and not one of the 8 known unhashed bundle/map/type-declaration paths), all `wildcard`/`eq`) → `Cache-Control: public, max-age=31536000` (edge + browser TTL both one year)
2. `http.host eq "js.maytes.co"` and `/v*/checkout-button.*` (`wildcard`) and **not** `/v*.*.*/checkout-button.*` → `Cache-Control: public, max-age=300`
...
**A real bug this design already had, caught by review before it shipped (twice — the second pass caught what the first missed):** `/checkout-button.*.*` (the hash-pin pattern) requires two literal dots, which correctly excludes the bare unhashed `checkout-button.js`/`.mjs`/`.cjs` (one dot) — but every OTHER unhashed, every-build-changes file `tsup`/`hash-and-sri.mjs` also write to `dist` root happens to have exactly two dots as well, the same count as a genuinely hashed file like `checkout-button.8c71493e.js`: the three bundles' own source maps (`checkout-button.js.map` etc.) and the two TypeScript declaration files `tsup`'s DTS build step produces (`checkout-button.d.ts`, `checkout-button.d.cts`) — the latter caught only on a second review pass after the first fix (which only excluded the three `.map` files) had already shipped and gone live. All 8 are exactly as mutable as the bundle they accompany and must not get a year-long cache. Fixed by explicitly excluding all 8 known unhashed paths (`UNHASHED_BUNDLE_PATHS` in `scripts/lib/cdn-cache-rules.mjs`) from the hash-pin match, via `eq`, rather than relying on the wildcard's dot-count alone. This list is a closed set describing this build's actual `dist/` output (verified against a real `npm run build` + `ls dist/`) — if `tsup`'s output ever changes shape, this list needs a matching update, since nothing currently derives it automatically from the build.

**Live-verified limitations, both discovered by actually running this against the real `maytes.co` zone:**
- The Ruleset Engine's `set_cache_settings` action has no field for the literal `immutable` directive — `action_parameters.cache_control_directives` (what an earlier draft of this design assumed) is rejected outright (`"invalid JSON: unknown field \"cache_control_directives\""`). Pins get `max-age=31536000` via `edge_ttl`/`browser_ttl` only, not the `immutable` token. `max-age` alone still means nothing is fetched again until the year expires for a normal navigation — but `immutable` specifically also skips the conditional revalidation request some browsers (Firefox in particular) send on a user-initiated hard reload; without it, a shopper who force-refreshes within the pin's `max-age` window can still trigger a 304 round-trip the doc previously implied wouldn't happen. Delivering `immutable` literally would need a separate Response Header Transform Rule (`http_response_headers` phase) layered on top — not implemented here; a follow-up if it's ever worth the extra API call. The rule's id, `checkout-button-immutable-pins`, still names the original intent rather than the delivered behavior — left as-is deliberately: `ensure-cdn-cache-rules.mjs` matches existing rules by that exact id, so renaming it would make the script treat the live rule as unowned and add a duplicate under the new name rather than updating it in place, orphaning the old one on the zone. Not worth that risk for a label.
- The `matches` (regex) operator — the original design for these two expressions — requires a Cloudflare Business plan or a WAF Advanced add-on (`"not entitled: the use of operator Matches is not allowed"`), which this zone doesn't have. Rewritten to use `wildcard` instead (available on every plan), which can't express character classes or quantifiers directly, but can still disambiguate the two path shapes because `wildcard`'s `.` is a literal character (unlike regex) and `*` is the only special token: requiring two literal dots before `/checkout-button.` is exactly what a full SemVer segment has and a bare major-only segment doesn't. Rule 2 additionally excludes anything Rule 1's pattern already matches, since `/v*/checkout-button.*` alone would also match the SemVer form (`*` matches across `/`, Cloudflare's wildcard isn't path-segment-aware). This has been verified conceptually against every path shape this CDN actually produces, but — like the fields above — it needed a live run against the real zone to catch the plan-entitlement issue in the first place, so treat the exact wildcard behavior as confirmed only once a deploy has actually succeeded with it.
