...
1. `http.host eq "js.maytes.co"` and (`/v*.*.*/checkout-button.*` **or** `/checkout-button.*.*`, both `wildcard`) → `Cache-Control: public, max-age=31536000` (edge + browser TTL both one year)
