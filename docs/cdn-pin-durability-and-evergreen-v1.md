...
- The Ruleset Engine's `set_cache_settings` action has no field for the literal `immutable` directive — `action_parameters.cache_control_directives` (what an earlier draft of this design assumed) is rejected outright (`"invalid JSON: unknown field \"cache_control_directives\""`). Pins get `max-age=31536000` via `edge_ttl`/`browser_ttl` only, not the `immutable` token. Functionally this is close (nothing revalidates before a year passes either way), but delivering `immutable` literally would need a separate Response Header Transform Rule (`http_response_headers` phase) layered on top — not implemented here; a follow-up if it's ever worth the extra API call.
