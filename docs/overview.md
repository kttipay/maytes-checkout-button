...
> For the merchant-facing integration guide, see [`INTEGRATION.md`](../../INTEGRATION.md) at the repo root. This document covers internals.
...
| Default mode (`redirect`) | Same-window navigation to the hosted checkout; `maytes:checkout-redirected` (`detail.url`). |
| `mode: 'popup'` on desktop | Blank popup opened synchronously, branded loader painted, then `popup.location.replace(url)`; `maytes:checkout-opened`. |
| `mode: 'popup'` on mobile | Behaves as `redirect` (same-window navigation). |
| Popup blocked in `mode: 'popup'` | Same-window redirect fallback; `maytes:checkout-redirected` (`detail.url`). No `failed` event. |
...
├── overlay.ts      # full-screen <dialog> with Maytes logo + spinner; iframe-aware host
...
- **`scripts/hash-and-sri.mjs`** — runs after `tsup`, writes `dist/integrity.json`, copies bundles to hashed filenames, and (when the package is publishable) rewrites the SRI block in `CHANGELOG.md` for the current `package.json` version. While the package is `private`, the CHANGELOG injection is skipped so gated builds don't dirty the file. SRI tables are machine-generated.
...
| `errors.test.ts` | 8 | `MaytesError` is an `Error`, code/message/name/stack present, `MaytesErrorCode.Config === 'CONFIG'`, code surface is `{ Config }` only |

A 7th file, `changelog.test.ts` (6 tests), unit-tests the release-script changelog slicer (`scripts/lib/changelog.mjs`), not SDK behaviour.
